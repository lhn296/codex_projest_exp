# ESP32 SPI LCD Display Template

基于 `ESP-IDF 5.5.3` 的 ESP32-S3 学习工程，用来完成 `I2C + XL9555 + SPI LCD` 的板载输入输出与显示模板练习，并继续复用现有统一事件架构。
...
- 当前版本：`v1.4.1`
- 目标芯片：`ESP32-S3`
- 当前阶段：`v1.4.1 LCD Display Service Optimize`
...
7. 观察 LCD 首页，确认版本号、阶段名、LED 状态、蜂鸣器状态、最近事件会刷新。
8. 连续操作不同按键时，观察页面局部区域刷新是否正常，不再每次整页重绘。
9. 观察串口日志，确认能看到按键名称、手势类型、LED / 蜂鸣器 / 显示业务处理过程。
